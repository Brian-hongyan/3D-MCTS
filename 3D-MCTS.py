...
def assign_iso(frag_id, Del_plus, start=False):
    '''
    根据重原子类型、芳香性等进行区分，可以先参考brics的文章，如果文章过于复杂，可以自己总结几类比较明显的，没必要总结很全。
    :return:
    返回一个全新的结构文件
    '''
    if start == False:
    ...
    def next_state(self):
        if self.type == 0:
            ...
            if len(ids_degs_smiles) > 0:
                ...
                ids_degs_scores = []
                scores = []
                for i in ids_degs_scores_smiles:
                ...
                if len(scores) > 0:
                    nextmove = ids_degs_scores_smiles[roulette(scores)]
    ...
    def next_states(self):
        if self.type == 0:
            ...
            if len(ids_degs_smiles) > 0:
                ...
                # ids_degs_scores_sorted = sorted(ids_degs_scores, key=lambda x: x[2])
                # ids_degs_scores_smiles = ids_degs_scores_sorted
                ids_degs_scores_smiles = ids_degs_scores
                ...
                for i in ids_degs_scores_sorted:
                    try:
                        if len(ids_degs_scores_smiles_dic[i[0]]) < 1:
                            # if len(ids_degs_scores_smiles_dic[i[0]]) < 24:
...
def UCTSEARCH(budget, root, start_score=0, num_moves_lambda=None):
    ...
    for iter in range(int(budget)):
        if (time.time() - start) > 4 * 3600 or len(GOOD_SCORES) > 200:
            break
        else:
